...
| B-016 | P1 | `environment.py:_assist_stalled_agent` | TTL prevents immediate duplicate help, but assistance has no causation chain/depth bound. | WorkItem cause/depth fields and loop rejection. | A→B→A help cycles across repeated stalls. | Reciprocal/depth-exhausted assistance becomes NEEDS_HUMAN without another task. | migrated |
| B-017 | P1 | `progress.py:ProgressTracker` | Signature is mostly repeated text; structured condition/step/child/work progress is not the primary measure. | Structured progress snapshot with textual failure signature as one signal. | False stalls or missed progress. | Completed step/artifact/condition resets no-progress even with repeated summary. | migrated |
| B-018 | P1 | persistence of EventBus and active WorkItems | Event history is diagnostic-only and active work recovery has no explicit restart transition/lease. | Persist durable causation/work state and reconcile active work on boot. | Duplicate delegation or work stuck ACTIVE after crash. | Crash/restart never executes completed work twice; stale ACTIVE work is explicitly recovered. | migrated |
| B-019 | P2 | `Environment.request_model_inference` | Broad external direct-inference entry point uses explicit telemetry but no TaskPacket. | Keep for human/architect compatibility; require bounded packet or documented exemption. | Context metadata is weaker than normal agent calls. | Direct request remains bounded and telemetry-complete. | accepted exemption: human/architect direct request, telemetry-complete, bounded by the caller |
| B-020 | P2 | Markdown `memory.md`, `context.md`, `WorldContext` | Compatibility projections can still influence prompts alongside structured state. | Make structured state authoritative; label projections and select narrowly. | Duplicate/stale facts in prompt, not state corruption. | Structured fact wins over conflicting projection text. | migrated: projection sections are labelled secondary and a projected line naming a belief's key or repeating a belief is left out of the prompt |
| B-021 | P0 | `goal_manager.py:GoalManager.refresh`, `mark_stalled` | Found while completing M4: a STALLED goal was skipped by refresh and excluded from selection, so nothing ever made it runnable again -- an agent's standing goal would die on its first stall. | Stall pauses the goal (cooldown doubling per stall, capped at 1h), then refresh returns it to RUNNABLE; a one-shot goal that stalls 3 times fails; the progress tracker restarts its count. | Silent permanent loss of an agent's work. | Stalled goal is back in line after the pause; one-shot goal fails after the stall limit. | migrated |
| B-022 | P1 | `memory.py:AgentMemory.compact` | Found by benchmark scenario G: the whole memory overflow (200 KB) went to the summarizer in one prompt, and keeping the newest third by count left the file over budget so it compacted again every cycle. | Keep the newest entries that fit half the memory budget; send the summarizer only what fits the prompt budget and record how many were dropped. | A 4k-context server silently truncates the compression prompt; one extra model call per cycle. | Compression prompt within budget; file within budget after one compaction. | migrated |
