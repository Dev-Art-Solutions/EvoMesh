...
- [ ] Render week-plus durations in `humanize_duration` using the true second-counts per unit
    ...
    1. [ ] src/evomesh/humanize.py `humanize_duration` -- replace the four `divmod` divisors with the correct number of seconds per unit (weeks → `_DAYS * _HOURS * _MINUTES * _WEEKS`, days → `_DAYS * _HOURS * _MINUTES`, hours → `_HOURS * _MINUTES`, minutes → `_MINUTES`) so the bucket counts are no longer shifted by one level.
