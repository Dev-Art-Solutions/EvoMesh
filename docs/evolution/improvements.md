...
- [ ] Fix `phase_label` to return `None` for an empty/whitespace/`None` phase, matching its documented contract ("empty or `None` -> `None`")
    ...
    >         return text or None
    1. [ ] src/evomesh/phase_label.py `phase_label` -- in the unknown-phase fallback, replace `return text[:1].upper() + text[1:] or "Unknown"` with `return text or None` (so empty/whitespace/`None` yields `None`), keeping the unknown-but-non-empty case returning `"Unknown"`.
