...
- [x] Compute each agent's stale goals once in `Environment.status`
    ...
    1. [x] src/evomesh/environment.py `Environment.status` -- compute each agent's stale-goal count once (e.g. build `{name: count}` first, then keep only non-zero counts) instead of calling `stalled_goals` twice per agent; the reported mapping must stay the same.
