...
**An agent can write its own, too.** `/learn grant <agent>` (needs `/harness grant` as well — `learn_skills: true` on an agent template grants both at once) adds `learn_skill` to that agent's own harness jobs, reactive chat reply and cyclic plan step alike: `learn_skill(name, description, body)`. Unlike the harness's own `write` tool — confined to the job root — this reaches the mesh-wide `skills/` directory through `SkillRegistry.install()`, the identical one step `/skill install` uses, so a written skill is real, well-formed, and discoverable immediately, never a draft. The one thing on top: an agent may only overwrite a skill it wrote itself (`created_by: agent:<id>`) — a name collision with a human-curated skill is refused, never silently replaced. Both `_respond_through_harness` and `through_harness` (bdi.py) nudge a granted agent to call it, but only *after* actually using a procedure not already covered by one of its own skills — never for one only planned. `/learn revoke <agent>` takes it back; `/learn status <agent>` shows whether it is currently granted. `news-watcher` ships with this on (see its `AGENT.md`): ask it for headlines in a format `news-report-export` does not already name, and — model willing — the next such request may not need this README updated for it.
