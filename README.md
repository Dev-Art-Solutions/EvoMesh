...
**Only one Control Center per checkout.** A second launch against the same `root` (a double-click while one is already open, say) is refused with a short message instead of opening a second window — a named OS-level `Mutex`, keyed off `root` and held for the process's whole life, the same reasoning `src/evomesh/singleton.py` already applies to the mesh process itself. Two Control Centers pointed at two *different* checkouts remain a legitimate, separate setup; only a second one against the same data is refused.
...
**That path has to actually be reachable from the job's own root, which is not always the mesh's project tree.** A system agent's harness job runs rooted at the project root, where `skills/` genuinely lives — but a non-system agent's own job runs rooted at its own playground, where the catalog's `skills/<name>/SKILL.md` line names a file that simply is not there. Found live: NewsAnalyzer's own recurring goal called `read` on its own `news-impact-analysis` skill, got back "does not exist", and improvised a full narrated report instead of the one-line-per-instrument format the skill actually specifies — which then matched nothing `report_pattern` was looking for, so a real, correctly-judged signal was silently never announced. `ToolContext.skills_root` (the mesh's project root, threaded through every job) is what fixes it: `read`/`grep`/`ls` fall back to the mesh-wide `skills/` directory when a path is not found inside the job's own root, and the `FilesystemPolicy` grant check is skipped for that fallback the same way it already is for the job's own root — reads only. `edit`/`write`/`delete` never get this fallback, so `learn_skill`/`patch_skill` stay the only way anything actually changes a skill.
...
**Installing one is the whole market mechanism.** `/skill install <path-or-url-or-directory>` accepts a lone `SKILL.md` — local file or fetched over HTTP — or a directory bundling scripts alongside one; either way it checks the frontmatter parses, writes it under `skills/<name>/`, and it is discoverable immediately, no restart. A directory install replaces a previous one of the same name outright, so a stale bundled file from an earlier version never survives a reinstall.

**An agent can write its own, too.** `/learn grant <agent>` (needs `/harness grant` as well — `learn_skills: true` on an agent template grants both at once) adds `learn_skill` and `patch_skill` to that agent's own harness jobs, reactive chat reply and cyclic plan step alike: `learn_skill(name, description, body)` creates or fully rewrites one, `patch_skill(name, old_text, new_text)` fixes one exact piece of a skill this same agent already wrote — the identical unique-match contract the harness's own `edit` tool uses on a real file, cheaper than resending a whole body through `learn_skill` for a small correction. Unlike the harness's own `write`/`edit` tools — confined to the job root — both reach the mesh-wide `skills/` directory through `SkillRegistry.install()`, the identical one step `/skill install` uses, so a written skill is real, well-formed, and discoverable immediately, never a draft. Both `_respond_through_harness` and `through_harness` (bdi.py) nudge a granted agent to call one of them, but only *after* actually using a procedure not already covered by one of its own skills — never for one only planned. `/learn revoke <agent>` takes it back; `/learn status <agent>` shows whether it is currently granted. `news-watcher` ships with this on (see its `AGENT.md`): ask it for headlines in a format `news-report-export` does not already name, and — model willing — the next such request may not need this README updated for it.

**Two safety nets sit underneath both tools.** First, an agent may only overwrite a skill it wrote itself (`created_by: agent:<id>`) — a name collision with a human-curated skill is refused, never silently replaced, re-checked again at approval time below in case the landscape moved. Second, `scan_skill_content()` (skills.py) runs on *every* write to `skills/` — a human's own `/skill install` included, not only an agent's — and refuses text that looks like a hardcoded credential (an API-key shape, a private key block, `password: "..."`) or a known instruction-override phrase ("ignore all previous instructions" and the like), so neither a leaked secret nor a prompt-injection payload staged for some future reading of the skill ever reaches disk in the first place.

**A human can require review before either tool writes anything.** `harness.skill_write_approval: true` in `evomesh.yaml` (off by default) makes `learn_skill`/`patch_skill` stage their proposed content instead of writing it — `/learn review` lists what is waiting (agent, kind, one-line summary), `/learn approve <n>` commits it exactly as staged (through the identical `install()` call, so nothing changes between what was reviewed and what lands), `/learn reject <n>` discards it untouched. Staged writes are in-memory only, like the harness queue's own jobs — restarting the mesh loses whatever was awaiting review.
...
**A `fetch` tool exists too, and it needs a companion install.** It fetches a URL and returns its main content as Markdown — via [Scrapling](https://github.com/D4Vinci/Scrapling), which is not a dependency of this project (see [scripts/install-scrapling.ps1](scripts/install-scrapling.ps1) / `.sh`) and is not offered to the model at all until `scraping.executable` in `evomesh.yaml` points at it. Static HTTP by default; `dynamic: true` renders with a real headless browser instead, for a page whose content JavaScript builds client-side — only present if the install script ran with `-WithBrowser` / `--with-browser` (a few hundred MB, one time), or `scrapling install` was run by hand from that venv.
...
**A human waiting on a reply cuts the line.** With one worker (the default, and the honest ceiling on a single GPU), a chat message otherwise sits behind whatever the Evolver's own pipeline happened to queue first — found live, the exact way it reads from the outside: "NewsWatcher doesn't respond." `HarnessQueue` orders on `(priority, job number)`, not submission order alone: `_respond_through_harness` (bdi.py) submits a reactive chat reply with `priority=True`, so it is picked up before any already-queued background job (the Evolver's pipeline, another agent's own plan step) — never *instead* of one already running, since nothing here preempts a job mid-flight. `/harness status` marks a priority job in its listing so it is visible, not just faster. `HarnessGateway.submit(..., priority=True)` is the one knob; nothing else in the queue changed, and `harness.workers: 2+` remains the separate, independent way to get real concurrency where the underlying model server can actually serve it.

**A reactive chat reply carries the last few messages, not just the one that just arrived.** `_respond_through_harness` used to submit *only* `message.content` — a follow-up like "send it as a PDF" names no content of its own at all, and a job that cannot see "give me the last 10 news" two messages back has nothing to build one from. Found live: asked what a bare "send it as PDF" should contain, then — given "the 10 news" with still no memory of "as a PDF" — the agent just repeated the headlines as chat text again instead of ever reaching `document_write`. The job's objective now includes up to the last three prior messages from `context.inbox` (the same history the plain non-harness fallback already had through `render_inbox()`) ahead of the current one, so a follow-up has something to resolve against — the agent still has to re-fetch and re-build its own answer, but at least knows what "it" refers to.
